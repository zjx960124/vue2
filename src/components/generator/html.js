...
const tags = {
  ...
  'el-upload': el => {
    ...
    const withCredentials = el['with-credentials'] === false ? ':with-credentials="false"' : ''
    ...
    if (child) child = `\n${child}\n` // 换行
    return `<${tag} ${ref} ${fileList} ${action} ${autoUpload} ${withCredentials} ${multiple} ${beforeUpload} ${listType} ${accept} ${name} ${disabled}>${child}</${tag}>`
  },
  tinymce: el => {
    const {
      tag,
