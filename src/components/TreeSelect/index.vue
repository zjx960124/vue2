<!--
     ...
     *              </tree-select>
      <select-tree
     props 字段映射如下，值填写你实际字段，可拓展字段。最终返回主键以及选择对象【全部字段】
                        id:'id',//可改成自己对应主键【改值】
                        label: 'name',// 显示名称
...
export default {
  ...
  computed: {
    treeData() {
      // 若非树状结构，则转化为树状结构数据
      return JSON.stringify(this.data).indexOf(this.props.children) !== -1
        ? this.data
        : this.switchTree()
    }
  },
    ...
    buildTree(data, id) {
      const fa = (id) => {
        ...
        for (let i = 0; i < data.length; i++) {
          const n = data[i]
          if (n[this.props.pid] === id) {
            n[this.props.children] = fa(n[this.props.id])
            temp.push(n)
          }
        }
        return temp
